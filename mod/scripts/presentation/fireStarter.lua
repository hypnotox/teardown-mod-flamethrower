-- FireStarter (presentation): the pilot flame at the fire-starter shape. Pure
-- local FX -- particles, a point light, and ammo-gated emissive.
FireStarter = {}
...
-- Ammo-gated pilot flame: emissive on + particles while there is ammo, off when
-- empty. (Was inlined in the old Flamethrower:tick.)
function FireStarter:tick()
    local shape = self:getShape()

    if State:hasAmmo() then
        SetShapeEmissiveScale(shape, 0.5)
        self:spawnParticles()
    else
