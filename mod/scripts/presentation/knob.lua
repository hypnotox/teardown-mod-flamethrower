-- Knob (presentation): renders the knob shape's rotation from the authoritative
-- velocity (State.flameVelocity), shows emissive while firing, and reads the
-- adjust keybinds to produce a velocity-change delta handed to Simulation. The
-- value itself is owned by State / Simulation; this module never stores or clamps it.
Knob = {
    changePerSecond = 10,
    keybinds = {
        decrease = 'leftarrow',
...
-- Reads keybinds from the registry, falling back to defaults.
...
function Knob:tick()
    ...
    if InputDown(self.keybinds.decrease) and not InputDown(self.keybinds.increase) then
        Simulation:applyVelocityDelta(-change)
...
function Knob:applyRotation(shape)
    ...
    axisTransform.rot = QuatEuler(0, 0, State.flameVelocityDefault - State:getFlameVelocity())
